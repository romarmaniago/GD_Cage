...
function updateChangeStatusRollerReturnSection() {
	...
	if (selectedStatus == '1' && (currentRequiredTotal > 0 || isPendingResolve)) {
		...
		// Pending games settle the balance via Loss / Deposit / Cash Paid instead of returning chips.
		var showPendingResolveForm = isPendingResolve && currentRequiredTotal > 0;
		$('#roller-chips-return-inputs').toggle(currentRequiredTotal > 0 && !showPendingResolveForm);
		$('#pending-resolution-section').toggle(showPendingResolveForm);
		if (showPendingResolveForm) {
			$('#txtReturnRollerNN, #txtReturnRollerCC').val('');
			updatePendingResolveTotal();
		}
		$('#pending-resolve-status-banner').hide();
	} else {
		$('#txtReturnRollerNN').val('');
...
function parsePendingResolveAmount(selector) {
	return parseFloat(String($(selector).val() || '').replace(/,/g, '').trim()) || 0;
}

function getPendingResolveAmounts() {
	var loss = parsePendingResolveAmount('#txtPendingLossAmount');
	var deposit = parsePendingResolveAmount('#txtPendingDepositAmount');
	var cash = parsePendingResolveAmount('#txtPendingCashPaid');
	return { loss: loss, deposit: deposit, cash: cash, total: loss + deposit + cash };
}

function updatePendingResolveTotal() {
	var required = parseFloat($('#modal-change_status').data('requiredReturnTotal')) || 0;
	var amounts = getPendingResolveAmounts();
	var $total = $('#pending-resolve-total');
	$total.text(amounts.total.toLocaleString('en-US'));
	$total.toggleClass('is-match', amounts.total > 0 && Math.abs(amounts.total - required) < 0.001);
	$total.toggleClass('is-mismatch', amounts.total > 0 && Math.abs(amounts.total - required) >= 0.001);
}

function resetPendingResolveForm() {
	$('#txtPendingLossAmount, #txtPendingDepositAmount, #txtPendingCashPaid').val('');
	$('#pending-resolve-mode-buyin').prop('checked', true);
	$('#pending-deposit-balance-hint').hide().text('');
	$('#modal-change_status').removeData('pendingDepositBalance');
	populatePendingResolveAccountSelect('txtPendingDepositAccount', 'modal-change_status');
	updatePendingResolveTotal();
}

function submitPendingResolve($btn) {
	var $modal = $('#modal-change_status');
	var required = parseFloat($modal.data('requiredReturnTotal')) || 0;
	var amounts = getPendingResolveAmounts();
	var accountId = $('#txtPendingDepositAccount').val() || '';
	var accountLabel = $('#txtPendingDepositAccount option:selected').text() || '';
	var mode = $('input[name="resolve_mode"]:checked', $modal).val() || 'buyin';
	var available = $modal.data('pendingDepositBalance');

	function stop(icon, title, html) {
		$btn.prop('disabled', false).html('Save');
		Swal.fire({ icon: icon, title: title, html: html });
	}

	if (amounts.total <= 0 || Math.abs(amounts.total - required) > 0.001) {
		stop('error', 'Amount mismatch',
			'Total Loss Amount (<strong>' + amounts.total.toLocaleString('en-US') + '</strong>) must equal Required RETURN (<strong>' +
			required.toLocaleString('en-US') + '</strong>).');
		return;
	}
	if (amounts.deposit > 0 && !accountId) {
		stop('warning', 'Account required', 'Please choose an account for the deposit amount.');
		return;
	}
	if (amounts.deposit > 0 && available != null && amounts.deposit > (parseFloat(available) || 0)) {
		stop('error', 'Insufficient balance',
			'Deposit amount exceeds the account\'s available balance (<strong>' + parseFloat(available).toLocaleString('en-US') + '</strong>).');
		return;
	}

	var rows = [['Type', mode === 'new_game' ? 'New Game' : 'Additional Buy-in']];
	if (amounts.loss > 0) rows.push(['Loss Amount', amounts.loss.toLocaleString('en-US')]);
	if (amounts.deposit > 0) rows.push(['Deposit (' + accountLabel + ')', amounts.deposit.toLocaleString('en-US')]);
	if (amounts.cash > 0) rows.push(['Cash Paid', amounts.cash.toLocaleString('en-US')]);
	rows.push(['Total Loss Amount', amounts.total.toLocaleString('en-US')]);

	SwalConfirm.fire({
		title: mode === 'new_game' ? 'Confirm New Game' : 'Confirm Additional Buy-in',
		rows: rows,
		message: 'Roller chips will be returned and the pending game will be ended. Proceed?',
		...
		$.ajax({
			url: '/game_list/pending_resolve',
			type: 'POST',
			data: {
				pending_game_id: $('.txtGameId', $modal).val() || game_id,
				resolve_mode: mode,
				txtLossAmount: amounts.loss,
				txtDepositAccountId: amounts.deposit > 0 ? accountId : '',
				txtDepositAmount: amounts.deposit,
				txtCashPaid: amounts.cash,
			...
			success: function (res) {
				Swal.fire({
					icon: 'success',
					title: 'Saved',
					text: res.message || 'Pending game resolved.',
					timer: 2200,
...
function fetchAccountDepositBalance(accountId, callback) {
	if (!accountId || typeof callback !== 'function') return;
	$.ajax({
...
function showGameListModal($modal, options) {
	if (!$modal || !$modal.length || $modal.hasClass('show')) {
		return;
	}
	...
	if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
		bootstrap.Modal.getOrCreateInstance(el, {
			focus: false,
			backdrop: opts.backdrop != null ? opts.backdrop : 'static',
			keyboard: opts.keyboard != null ? opts.keyboard : false
		}).show();
		return;
	}
	$modal.modal('show');
...
function buildGameRemarksButton(row, big) {
	...
	return (
		...
		' style="font-size:' + fontSize + ' !important; margin-right: 0;">' +
		'<i class="fa fa-comment-alt"></i></button></div>'
	);
}

function buildGameReceiptButton(row, big) {
	var sizeClass = big ? ' action-btn-square-lg' : '';
	var fontSize = big ? '24px' : '14px';
	return (
		'<div class="btn-group" role="group">' +
		'<button type="button" class="btn btn-sm btn-success-subtle action-btn-square' + sizeClass + ' js-bs-tooltip-enabled"' +
...
function resolveGameListAccountId(accountId, agentCode) {
	var id = parseInt(accountId, 10);
	if (id) return id;
	id = parseInt(agentCode, 10);
	return id || 0;
}

function applyGameListRowHtmlCells(row, data) {
	var htmlCols = [2, 3, 6, 8, 9, 11, 12, 13, 14, 15, 16, 17];
	htmlCols.forEach(function (colIdx) {
		var html = data[colIdx];
		if (html != null && typeof html === 'string' && html.indexOf('<') !== -1) {
			$('td:eq(' + colIdx + ')', row).html(html);
		}
	});
}
...
function buildGameAccountCell(accountId, agentCode, agentName) {
	...
	var label = code;
	var id = resolveGameListAccountId(accountId, code);
	return (
		'<span class="game-list-cell-truncate" title="' + escapeHtmlText(label) + '">' +
		'<a href="#" class="game-list-acct-link"' +
		' data-account-id="' + id + '"' +
		' data-agent-code="' + escapeHtmlText(code) + '"' +
		' data-agent-name="' + escapeHtmlText(name) + '">' +
		escapeHtmlText(label) +
		'</a></span>'
	);
}

function buildGameGroupBadge(row) {
	var groupName = row.group_name ? String(row.group_name).trim() : 'Main';
	var groupId = row.GROUP_ID != null ? row.GROUP_ID : '';
	if (!row.game_list_id || !canAssignGameGuest()) {
		return ' <span class="game-list-group-badge text-muted">' + escapeHtmlText(groupName) + '</span>';
	}
	return (
		' <button type="button" class="js-assign-game-group game-list-group-link"' +
		' data-game-id="' + row.game_list_id + '"' +
		' data-group-id="' + escapeHtmlText(String(groupId)) + '"' +
		' data-bs-toggle="tooltip" title="Change group">' +
		escapeHtmlText(groupName) +
		'</button>'
	);
}
...
function captureGameListExportRow(row, vals) {
	...
	var programEnd = row.GAME_ENDED ? moment(row.GAME_ENDED).format('YYYY-MM-DD') : '';
	window._gameListExportRows[row.game_list_id] = {
		program_date: programDate,
		game_start: gameStart,
		program_end: programEnd,
		game_end: gameEnded,
		agent_code: row.agent_code || '',
		group_name: row.group_name ? String(row.group_name).trim() : 'Main',
		guest_name: row.guest_name && row.guest_name !== '-' ? row.guest_name : '',
		membership_no: row.membership_no || '',
		game_type: row.GAME_TYPE || '',
		commission_percentage: row.COMMISSION_PERCENTAGE,
		game_id_label: buildCutoffGameIdPlainLabel(row),
		buyin: vals.buyin || 0,
		cashout: vals.cashout || 0,
		winloss: vals.winloss || 0,
		rolling: rolling,
		settlement: settlementDisplay,
...
$(function () {
	$(document).on('click', '#game_list-tbl .game-list-acct-link', function (e) {
		e.preventDefault();
		e.stopImmediatePropagation();
		var $link = $(this);
		var id = resolveGameListAccountId($link.attr('data-account-id'), $link.attr('data-agent-code'));
		if (!id || typeof window.account_details !== 'function') return false;
		window.account_details(
			id,
			$link.attr('data-agent-code') || '',
			$link.attr('data-agent-name') || ''
		);
		return false;
	});
	$(document).on('click', '#game_list-tbl .js-game-list-add-chg', function (e) {
		e.preventDefault();
		e.stopImmediatePropagation();
		var $btn = $(this);
		if (typeof openServices !== 'function') return false;
		openServices(
			parseInt($btn.attr('data-game-id'), 10),
			encodeURIComponent($btn.attr('data-agent-code') || ''),
			encodeURIComponent($btn.attr('data-guest-name') || ''),
			parseInt($btn.attr('data-game-status'), 10),
			parseInt($btn.attr('data-settled'), 10),
			parseInt($btn.attr('data-agent-id'), 10)
		);
		return false;
	});
});
...
function buildGameMembershipCell(row) {
	var membershipNo = row.membership_no != null ? String(row.membership_no).trim() : '';
	return escapeHtmlText(membershipNo || '-');
}
...
function loadAssignGameGroupSelect(currentGroupId, onReady) {
	var $groupSelect = $('#assign_game_group_select');
	...
	$.ajax({
		...
		method: 'GET',
		success: function (rows) {
			var groups = Array.isArray(rows) ? rows : [];
			groups.forEach(function (group) {
				$groupSelect.append($('<option>', { value: group.IDNo, text: group.NAME }));
			});
			if (currentGroupId) {
				$groupSelect.val(String(currentGroupId));
			}
			$groupSelect.select2({
				placeholder: 'Select group',
				allowClear: false,
				dropdownParent: '#modal-assign-game-group',
				width: '100%'
			});
...
function openAssignGameGroupDialog(gameId, currentGroupId) {
	if (!canAssignGameGuest()) {
		Swal.fire({ icon: 'warning', title: 'Not allowed', text: 'You cannot assign a group.' });
		return;
	}
	if (!gameId) {
		Swal.fire({ icon: 'warning', title: 'Missing data', text: 'Could not load this game.' });
		return;
	}

	$('#assign-group-game-id-label').text(gameId);
	$('#assign_group_game_id').val(gameId);

	loadAssignGameGroupSelect(currentGroupId, function () {
		$('#modal-assign-game-group').modal('show');
	});
}
...
$(document).ready(function () {
	...
	function openMergeSettlementModal(selectedIds) {
		...
		if (!$modal.length) {
			if (typeof Swal !== 'undefined') {
				Swal.fire({ icon: 'error', title: 'Modal not found', text: 'Multiple settlement modal is not loaded.' });
			}
			return;
		}
		...
		$('.merge-settle-checkbox:checked').each(function () {
			...
			totalRolling += parseMergeNumeric($row.find('td').eq(11).text());
	...
	$.fn.dataTable.ext.type.order['game-list-acct-group-pre'] = function (d) {
		if (!d) return '';
		var $wrap = $('<div>').html(typeof d === 'string' ? d : String(d));
		var groupText = $wrap.find('.game-list-group-link, .game-list-group-badge').first().text();
		return String(groupText || '').trim().toLowerCase();
	};
	...
	var dataTable = $('#game_list-tbl').DataTable({
		...
		columnDefs: [
			...
			{ targets: 4, className: 'col-membership', width: '1%' },
			{ targets: 5, className: 'col-type text-center', width: '68px' },
			{ targets: 6, className: 'text-center col-game-rate' },
			{ targets: 7, type: 'game-list-col2', className: 'text-center' },       // GAME # / game count: custom numeric sort
			{ targets: 8, className: 'col-buyin', width: '130px' },
			{ targets: 9, className: 'col-cashout', width: '130px' },
			{ targets: 10, className: 'col-winloss', width: '130px' },
			{ targets: 11, className: 'col-total-rolling', width: '130px' },
			{ targets: 12, className: 'text-center col-commission' },
			{ targets: 14, className: 'text-center col-total-settle' },
			{ targets: 15, className: 'text-center col-game-end' },
			{ targets: 16, className: 'col-roller-chips' },
			{ targets: 17, className: 'text-start col-action' },
		...
		createdRow: function (row, data, index) {
			applyGameListRowHtmlCells(row, data);
			...
			const gameListIdText = $('<div>').html(data[7]).text(); // assuming column 7 is GAME #
	...
	/** Rows for the grouped-header detailed export (see captureGameListExportRow), in the
	 *  table's current filter/search/sort order. Reads game_list_id off each row's action
	 *  buttons (data-game-id) rather than the DataTables data array, since the cells hold
	 *  rendered HTML, not the raw row objects. */
	function getGameListDetailedExportRows() {
		if (!$.fn.DataTable.isDataTable('#game_list-tbl')) return [];
		var dt = $('#game_list-tbl').DataTable();
		var out = [];
		var exportMap = window._gameListExportRows || {};
		dt.rows({ search: 'applied', order: 'applied' }).every(function () {
			var node = this.node();
			var idEl = node.querySelector('[data-game-id]');
			var gameId = idEl ? parseInt(idEl.getAttribute('data-game-id'), 10) : null;
			if (!gameId) return;
			var exp = exportMap[gameId];
			if (exp) out.push(exp);
		});
		return out;
	}
	...
	$('#btn-game-list-export').on('click', function (e) {
		e.preventDefault();
		if (!$.fn.DataTable.isDataTable('#game_list-tbl')) return;
		var rows = getGameListDetailedExportRows();
		if (rows.length === 0) {
			if (typeof Swal !== 'undefined') {
		...
		fetch('/game_list/export_xlsx_grouped', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			credentials: 'same-origin',
			body: JSON.stringify({ rows: rows, filename: outName })
		})
			.then(function (res) {
				if (!res.ok) {
	...
	window.showAccountSummaryView = function () {
		...
		dt.order([[2, 'asc']]); // Account view: sort by ACCT No (column 2)
		dt.draw();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                function addAccountRows() {
                    ...
                    dataTable.order([[2, 'asc']]); // Account view: sort by ACCT No (column 2)
                    dataTable.draw();
                    $('#game_list-tbl tfoot #GRAND_TOTAL_AMOUNT').text(grandAmount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }));
                    $('#game_list-tbl tfoot #GRAND_CHIPS_RETURN').text(grandChipsReturn.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }));
                    $('#game_list-tbl tfoot #GRAND_TOTAL_ROLLING').html(formatListAmount(grandRolling, 'signed'));
...
// let rowClass = isHighlighted ? 'highlight-row' : '';
                    ...
                    var btn_his = `<div class="btn-group" role="group">
                        <button type="button" onclick="showHistory(${row.game_list_id})" class="btn btn-sm btn-info-subtle action-btn-square js-bs-tooltip-enabled"
                            ...
                            style="font-size:16px !important; margin-right: 0;">
                            <i class="fa fa-history"></i>
                        </button>
                    </div>`;
                    var btn_remarks = buildGameRemarksButton(row, true);
                    var btn_receipts = buildGameReceiptButton(row, true);
					...
					var total_roller_return_cc = 0;
							...
							if (row.game_status == 2) {
								...
								cashout_td = '<button class="btn btn-link" style="font-size:11px;text-decoration: underline;" onclick="addCashout(' + row.game_list_id + ', ' + row.ACCOUNT_ID + ', ' + total_rolling_chips + ', ' + gameListAgentOnclickArgs(row.agent_code, row.guest_name) + ')">' + formatListAmount(total_cash_out_chips) + '</button>';
...
$('#edit_status').submit(function (event) {
	...
	// PENDING game with an outstanding roller balance: settled via Loss / Deposit / Cash Paid instead.
	if (isEndGameStatus(status) && $('#pending-resolution-section').is(':visible')) {
		submitPendingResolve($btn);
		return;
	}
...
function buildAddChgTd(gameListId, agentCode, guestName, addChgValue, gameStatus, settled, agentId) {
	...
	var guest = normalizeGameGuestName(guestName);
	var gid = parseInt(gameListId, 10);
	var gStatus = parseInt(gameStatus, 10);
	var settledVal = parseInt(settled || 0, 10);
	var agentIdVal = parseInt(agentId || 0, 10);
	return (
		'<button type="button" class="btn btn-link js-game-list-add-chg" style="font-size:11px;text-decoration: underline;"' +
		' data-game-id="' + gid + '"' +
		' data-agent-code="' + escapeHtmlText(agentCode || '') + '"' +
		' data-guest-name="' + escapeHtmlText(guest) + '"' +
		' data-game-status="' + gStatus + '"' +
		' data-settled="' + settledVal + '"' +
		' data-agent-id="' + agentIdVal + '">' +
		display +
		'</button>'
	);
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            function formatBuyinCell(val, transType) {
                ...
                // Pending-resolve Loss Amount buy-in (TRANSACTION = 7): not cash
                if (transType === 7) return '<span class="rolling-cell rolling-cell-loss">' + str + '</span>';
                return str;
            }
...
function openServices(id, agentCode, guestName, gameStatus, settled, agentId) {
	...
	showGameListModal($('#modal-services'), { keyboard: false });
...
$(document).ready(function () {
	...
	$(document).on('click', '.js-assign-game-group', function (e) {
		e.preventDefault();
		var $btn = $(this);
		openAssignGameGroupDialog(
			parseInt($btn.data('game-id'), 10),
			parseInt($btn.data('group-id'), 10) || null
		);
	});

	$('#assign_game_group_form').on('submit', function (event) {
		event.preventDefault();
		var gameId = parseInt($('#assign_group_game_id').val(), 10);
		var groupId = parseInt($('#assign_game_group_select').val(), 10);
		if (!groupId) {
			Swal.fire({ icon: 'warning', title: 'Group required', text: 'Please select a group before saving.' });
			return;
		}
		var $submitBtn = $('#submit-assign-game-group-btn');
		$submitBtn.prop('disabled', true);

		$.ajax({
			url: '/game_list/' + gameId + '/group',
			method: 'PUT',
			contentType: 'application/json',
			...
			success: function () {
				...
				Swal.fire({
					...
					$('#modal-assign-game-group').modal('hide');
				});
			},
			error: function (xhr) {
				Swal.fire({
					icon: 'error',
					title: 'Error',
					text: xhr.responseJSON?.error || 'Failed to save group.'
				});
			},
			complete: function () {
	...
	// Pending resolve form: Loss Amount + Deposit + Cash Paid must add up to the Required RETURN.
	$(document).off('input.pendingResolveTotal', '#pending-resolution-section .pending-resolve-amount')
		.on('input.pendingResolveTotal', '#pending-resolution-section .pending-resolve-amount', updatePendingResolveTotal);

	$(document).off('change.pendingResolveAccount', '#txtPendingDepositAccount')
		.on('change.pendingResolveAccount', '#txtPendingDepositAccount', function () {
			var accountId = $(this).val();
			var $modal = $('#modal-change_status');
			var $hint = $('#pending-deposit-balance-hint');
			$modal.removeData('pendingDepositBalance');
			$hint.hide().text('');
			if (!accountId) return;
			fetchAccountDepositBalance(accountId, function (balance) {
				if ($('#txtPendingDepositAccount').val() !== accountId) return;
				$modal.data('pendingDepositBalance', balance);
				$hint.text('Available balance: ' + parseFloat(balance).toLocaleString('en-US')).show();
			});
		});
					...
					if (rollerTransaction == 1) {
						...
						total_roller_return_cc += (res.ROLLER_CC_CHIPS || 0);
							...
							if (row.game_status == 2) {
								...
								total_rolling_td = buildTotalRollingTd(row.game_list_id, row.agent_code, row.guest_name, total_rolling_chips, false);
								cashout_td = formatListAmount(total_cash_out_chips);
                                var actionButtons = btn_remarks + btn_receipts + btn_his;
                                var acct_no_link = buildGameAccountCell(row.ACCOUNT_ID || row.account_no, row.agent_code, row.agent_name);
                                dataTable.row.add([`GAME-${row.game_list_id}`, acct_no_link, buyin_td, cashout_td, total_rolling_td, `${row.COMMISSION_PERCENTAGE}%`, net, winloss, status, actionButtons]).draw();
							} else {
...
/** Settlement is always single-game (no cross-game "Total" settlement), kept as a no-op
 *  so the handful of change-status call sites that refresh an open settlement modal don't
...
function fetchAccountBalanceTotal(accountId) {
	if (!accountId) {
	...
	return $.ajax({
		...
		var totalBalance = deposit_amount + marker_deposit_amount - withdraw_amount - marker_return;
		return !isNaN(totalBalance) ? totalBalance : 0;
	}).catch(function () {
		return 0;
	});
}
...
function fetchSettlementBalanceForAccount(accountId) {
	...
	fetchAccountBalanceTotal(accountId).then(function (totalBalance) {
		$('#SettlementBalance').val(totalBalance);
	});
}
...
function settlement_history(record_id, acc_id) {
    ...
    function loadServicesTotal() {
        $.ajax({ url: '/game_services/' + record_id, method: 'GET' })
            .done(function (list) {
                var totalsMap = {};
    ...
    function applySettlementButtonState() {
        var $btn = $settlementModal.find('#submit-settlement-btn');
        if (isSettled) {
            $btn.show().prop('disabled', true).text('Settled');
            $settlementModal.find('.deposit-cashout-row').hide();
        } else {
            $btn.prop('disabled', false).text('Settle').show();
            $settlementModal.find('.deposit-cashout-row').show();
        }
        if (typeof window.updateSettlementCopyButtons === 'function') {
            window.updateSettlementCopyButtons($settlementModal);
    ...
    // Single-game settlement: re-applies this game's own fetched metrics to the form.
    ...
    function reloadDataRecord() {
        $.ajax({ url: '/game_record_data/' + record_id, method: 'GET' })
            .done(function (data) {
                dataTable.clear();
                ...
                currentGameMetrics = computeGameSettlementMetricsFromRows(data);

                var accNo = (data[0].agent_code || '') + ' - ' + (data[0].agent_name || '');
    ...
    $('#submit-settlement-btn').off('click').on('click', function (e) {
        ...
        // Refresh computed values before reading them for confirmation
        renderSettlementView();
