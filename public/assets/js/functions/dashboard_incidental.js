$(document).ready(function () {
	if (!$('#modal-dash-incidental').length) return;
	...
	$.fn.dataTable.ext.search.push(function (settings, data) {
		if (!settings || !settings.nTable || settings.nTable.id !== 'dash-incidental-table') return true;
		if (!incidentalDateStart || !incidentalDateEnd) return true;
		...
		var start = new Date(incidentalDateStart.getFullYear(), incidentalDateStart.getMonth(), incidentalDateStart.getDate(), 0, 0, 0, 0);
		var end = new Date(incidentalDateEnd.getFullYear(), incidentalDateEnd.getMonth(), incidentalDateEnd.getDate(), 23, 59, 59, 999);
		return rowDate >= start && rowDate <= end;
	});

	function initializeDataTable() {
		if ($.fn.DataTable.isDataTable('#dash-incidental-table')) {
			$('#dash-incidental-table').DataTable().destroy();
		}

		dataTable = $('#dash-incidental-table').DataTable({
			pageLength: 10,
		...
		if (typeof window.bindDashServiceTableControls === 'function') {
			window.bindDashServiceTableControls('dash-incidental-table', 'btn-dash-incidental-new-record');
		}
	}
		...
		$.ajax({
			...
			success: function (services) {
				...
				(services || []).forEach(function (service) {
					if (!isIncidentalServiceType(service.SERVICE_TYPE)) return;
			...
			error: function (_xhr, _status, error) {
				console.error('Error loading dashboard Incidental data:', error);
			}
		});
	}
	...
	$('#btn-dash-incidental-new-record').on('click', function () {
		window.__dashServicePresetType = 'Incidental';
	});

	$(document).on('click', '#modal-dash-incidental .edit-service-btn', function () {
		var $btn = $(this);
		var payload = {
	...
	$(document).on('click', '#modal-dash-incidental .delete-service-btn', function () {
		var id = $(this).data('id');
		SwalConfirm.fire({
	...
	$(document).on('click', '#modal-dash-incidental .gamebook-notice-btn', function () {
