...
async function dropDirectionColumnIfExists(pool) {
	if (!(await tableExists(pool, 'services_category'))) return false;
	if (!(await columnExists(pool, 'services_category', 'DIRECTION'))) return false;

	await pool.execute('ALTER TABLE services_category DROP COLUMN DIRECTION');
	console.log('[services_category] Dropped column DIRECTION');
	return true;
}
...
async function ensureServicesCategorySchema(pool) {
	try {
		...
		await dropDirectionColumnIfExists(pool);
