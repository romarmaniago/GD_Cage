...
router.post('/add_marker_settlement', async (req, res) => {
	...
	async function insertSettlementRecord(sourceOverride, amountOverride) {
		...
		const userRemarks = (remarks || '').toString().trim() || null;
		const remarksValue = userRemarks || 'CREDIT RETURN';
		const insertQuery = `
            INSERT INTO account_ledger (ACCOUNT_ID, TRANSACTION_ID, TRANSACTION_TYPE, AMOUNT, ENCODED_BY, ENCODED_DT, REMARKS, AUTO_REMARKS, TRANSACTION_DESC)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

		const [ledgerResult] = await pool.execute(insertQuery, [accountId, optTransType, 3, amount, req.session.user_id, date_now, userRemarks, 'CREDIT RETURN', returnSourceDesc]);
